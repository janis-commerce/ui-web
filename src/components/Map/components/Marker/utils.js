			...
			${
				index
					? `
						...
						/>
						<text x="${mainSizeWidth / 2 + padding}" y="${mainSizeHeight -
							3 +
							separation +
							padding}" text-anchor="middle" dominant-baseline="middle" class="text">${index}</text>
					`
					: `<rect
...
export const getSvgUrl = ({ color, icon, size, label, iconColor }, index) =>
	icons['alarm_clock']
		? getIconSvgTemplate(color, 'alarm_clock', size, iconColor, index)
