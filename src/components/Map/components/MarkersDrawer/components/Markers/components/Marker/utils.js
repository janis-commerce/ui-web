...
export const getDefaultSvgTemplate = (color, label) => {
	...
	const template = `data:image/svg+xml;utf-8, \
		<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<text x="49%" y="43%" text-anchor="middle" dominant-baseline="middle" font-size="12" font-family="Roboto" font-weight="1000" fill="white" >${labelStr}</text>
			<defs>
...
const getIconSvgTemplate = ({ color, icon, size, iconColor, index = null }) => {
	...
	const encodedColor = encodeColor(color);
	const encodedIconColor = iconColor && iconColorTheme(iconColor);
	const validIconColor = encodedIconColor || encodeColor(colors.white);
	...
	const template = `data:image/svg+xml;utf-8, \
		...
		>
				...
				index
					? `
						<rect
							...
							height="24"
							rx="12"
							fill="${encodedIconColor || encodedColor}"
						/>
			...
			<g class='iconPath'>
				<path fill-rule="evenodd" clip-rule="evenodd" d="${transformedPath}" fill="${validIconColor}" class="iconPath"/>	
			</g>
			<defs>
				<filter
...
export const isValidAnimation = (animation = {}) => {
	...
	if (!animation || !isObject(animation)) return false;
