...
- `config.minZoom`/`config.maxZoom` (opcionales): pass-through a `<ReactFlow>`; sin declarar, se usan los defaults del componente (`0.5`/`2`).
- Clamp de `initialZoom` contra `minZoom`/`maxZoom` **efectivos** (leídos del store, no de `config`), que reflejan los defaults del componente ya resueltos por RF.
...
- Validación de `config.minZoom`/`config.maxZoom` antes de pasarlos a `<ReactFlow>`: si alguno es `<= 0`, o si el rango está invertido (`maxZoom <= minZoom`), se ignoran **ambos** valores (warning en desarrollo) y se cae a los defaults del componente (`0.5`/`2`).
...
- [ ] `minZoom`/`maxZoom` sin declarar usan los defaults del componente (`0.5`/`2`).
