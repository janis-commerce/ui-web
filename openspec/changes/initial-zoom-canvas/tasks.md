...
- [x] 2.5 Agregar `getEffectiveZoomBounds(minZoom, maxZoom)` (función pura de módulo, arriba del componente) que valida `config.minZoom`/`config.maxZoom` contra los valores **efectivos** (declarado o default del componente): si alguno es `<= 0`, o si el rango efectivo queda invertido (`maxZoom <= minZoom`), ignora ambos (warning en dev) y devuelve los defaults del componente (`DEFAULT_MIN_ZOOM`/`DEFAULT_MAX_ZOOM` = `0.5`/`2`). Calcular `effectiveMinZoom`/`effectiveMaxZoom` con `useMemo` dentro de `Canvas` y pasarlos a `<ReactFlow>` en vez de los crudos de `config` (snippet exacto en `design.md`).
...
- [ ] 3.6 Test: sin `minZoom`/`maxZoom`, los límites efectivos son `0.5`/`2` (*"minZoom/maxZoom sin declarar usan los defaults del componente"*).
...
- [x] 3.13 Test: `minZoom`/`maxZoom` con valor `<= 0` (ej. `minZoom = -1`, o `maxZoom = 0`) → se ignoran ambos, los límites efectivos son los defaults del componente (`0.5`/`2`) (*"minZoom o maxZoom con valor <= 0"*).
- [x] 3.14 Test: `maxZoom <= minZoom` (ej. `minZoom = 5`, `maxZoom = 2`) → se ignoran ambos, los límites efectivos son los defaults del componente, y el clamp de `initialZoom` (si está definido) se calcula contra esos defaults (*"maxZoom menor o igual a minZoom"*).
